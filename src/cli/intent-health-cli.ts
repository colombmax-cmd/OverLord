...
async function probeLocalRuntime(fetchImpl: typeof fetch, baseUrl: string, timeoutMs: number): Promise<{
  ...
  try {
    ...
    return { ok: false, reason: normalizeProbeErrorReason(error) };
  } finally {
    clearTimeout(timeoutHandle);
  }
}

function normalizeProbeErrorReason(error: unknown): string {
  ...
  const causeMessage = extractNestedCauseMessage(error.cause);
  if (causeMessage) {
    return causeMessage;
  }

  return directMessage || 'fetch failed';
}

function extractNestedCauseMessage(cause: unknown): string | null {
  if (!cause) {
    return null;
  }
  ...
  if (cause instanceof Error) {
    ...
    return extractNestedCauseMessage(cause.cause);
  }
