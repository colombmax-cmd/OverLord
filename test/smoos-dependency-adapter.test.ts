...
test('createSmoosDependencyAdapter auto-detects a factory export from a dependency module', async () => {
  ...
  const capability = await adapter.checkCapability({ actorId: 'user-1', capability: 'intent:read' });
  assert.equal(capability.allowed, true);

  const events = await adapter.readEvents({ stream: 'intent' }, 'intent:read');
...
test('createSmoosDependencyAdapter bridges the installed Smo.OS git dependency', async () => {
  const require = createRequire(import.meta.url);
  assert.doesNotThrow(() => require.resolve('smo-os/package.json'));
  ...
  try {
    ...
    await adapter.writeEvent({ title: 'bridge-to-smo-os' }, 'workflow:submit');
    const events = await adapter.readEvents({ stream: 'intent', limit: 10 }, 'intent:read');
    assert.equal(events.length, 1);
    assert.equal(events[0].type, 'overlord.memory/event_written');

    await adapter.emitAudit({
      actorId: 'user-1',
    ...
    const allEvents = await adapter.readEvents({ stream: 'intent', limit: 10 }, 'intent:read');
