...
Quick troubleshooting when `intent health` shows `Local runtime status: unreachable`:

```bash
# 1) verify the runtime is reachable
curl -i "$OVERLORD_LOCAL_LLM_BASE_URL/api/tags"

# 2) if connection is refused, start your local runtime service first (example with Ollama)
ollama serve

# 3) ensure the model exists locally
ollama pull "$OVERLORD_LOCAL_LLM_MODEL"
